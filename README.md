...
Note that usage of the `displayLabel` attribute in Cocina overrides the `label` that would ordinarily be used to group an item. It's possible for use of `displayLabel` to result in items being grouped together after relabeling, or to be split out into their own group.
...
If the data you have is a hash from parsed Cocina JSON, you can use `DisplayData.from_cocina`. You can optionally provide a `label` to use if the Cocina did not contain a `displayLabel` attribute:
...
For contributors, the underlying objects are `Contributor` instances, which provide access to the associated `Name` and `Role` objects:

```ruby
> record.contributor_display_data.first.objects.first.name.value
=> "Hearst Magazines, Inc."
```
