module CocinaDisplay
  module Concerns
    ...
    module Titles
      ...
      # @return [String, nil]
      def main_title
        primary_titles.first&.short_title
      end
      ...
      # @return [String, nil]
      def full_title
        primary_titles.first&.full_title
      end
      ...
      # @return [String, nil]
      def display_title
        primary_titles.first&.display_title
      end
      ...
      # @return [String]
      def sort_title
        primary_titles.first&.sort_title || "\u{10FFFF}"
      end
      ...
      # All titles marked primary, or those without a type if none are primary.
      # @return [Array<Title>]
      def primary_titles
        all_titles.filter { |title| title.primary? }.presence || all_titles.filter { |title| !title.type? }
      end

      # All titles except the primary titles and those without a type.
      # @return [Array<Title>]
      def secondary_titles
        (all_titles - primary_titles).reject { |title| !title.type? }
      end
