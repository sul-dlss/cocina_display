...
RSpec.describe CocinaDisplay::TitleBuilder do
  ...
  let(:main_title) { described_class.main_title(titles) }
  let(:full_title) { described_class.full_title(titles) }
  let(:additional_titles) { described_class.additional_titles(titles) }
  ...
  context "with single untyped title with simple value" do
    ...
    describe ".build" do
      ...
      context "with a partLabel" do
        subject(:builder_build) { described_class.build(titles, catalog_links: catalog_links, strategy: strategy, add_punctuation: add_punctuation) }
  ...
  context "with structuredValue is subtitle, part name, part number and partLabel" do
    ...
    let(:full_title) { described_class.full_title(titles, catalog_links: catalog_links) }
    let(:titles) do
      [
        {
          "structuredValue" => [
